...
(function (_global) {
                        ...
                        case '{':
                            ...
                            if (token.selectors !== void 0) {
                                ...
                                if (token.atValues && token.atValues.length) {
                                    let values = token.atValues.join(_this.selectorsSeparator);
    ...
    (function () {
        const ParseError = modules['Error'];
        const Builder = modules['Builder'];
                ...
                this._findTokens(validTokens).forEach(function (token) {
                    ...
                    switch (token.token) {
                        case '//':
                            ...
                            let end1 = _this2._css.indexOf('\n', start + 2);
                            let end2 = _this2._css.indexOf('\r', start + 2);
                        ...
                        case '{':
                            ...
                            if (!_this2.splitRules) {
                                if (selectorStart > blockStart) {
                                    let code = _this2._css.slice(blockStart, selectorStart).trim();
                                    if (code.length) {
                                        items.push({
                                            token: 'code',
                                            code: code,
                        ...
                        case '}':
                            ...
                            if (_this2.splitRules) {
                                ...
                                let code = _this2._css.slice(blockStart, token.index).trim();
                                if (code.length) {
                                    items.push({
                                        token: 'code',
                                        code: code,

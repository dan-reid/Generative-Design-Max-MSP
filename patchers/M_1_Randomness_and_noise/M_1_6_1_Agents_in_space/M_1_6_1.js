...
function Agent() {
		this.p = pc.create_vector(	pc.random(-spacesize_x, spacesize_x),
																pc.random(-spacesize_y, spacesize_y),
																pc.random(-spacesize_z, spacesize_z
																));
		...
		Agent.prototype.update = function() {
			...
			this.angle_y = pc.noise(	offset_x / Math.max(1, noise_scale),
																offset_y / Math.max(1, noise_scale),
																offset_z / Math.max(1, noise_scale)	) * noise_strength;

			// offset angle_z by another amount for variation
			this.angle_z = pc.noise(	offset_x / Math.max(1, noise_scale) + 10000,
																offset_y / Math.max(1, noise_scale) + 10000,
																offset_z / Math.max(1, noise_scale) + 10000	) * noise_strength;
			...
			if ( this.p.x < -spacesize_x || this.p.x > spacesize_x ||
					 this.p.y < -spacesize_y || this.p.y > spacesize_y ||
					 this.p.z < -spacesize_z || this.p.z > spacesize_z	) {
...
function Ribbon3d(p, n) {
	...
	Ribbon3d.prototype.draw_line_ribbon = function draw_line_ribbon(col, width) {
		...
		for(var i = 0; i < this.count-1; i++) {
			if(!this.is_gap[i] == true) {
				sketch.linesegment(	this.points[i].x, this.points[i].y, this.points[i].z,
